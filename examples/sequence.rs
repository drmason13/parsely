//! This example demonstrates some parsers that match multiple items sequentially
//! and escape certain characters.
//!
...
//!
//! The most important combinator used here is `many()`
//! which takes a range to describe how many times a parser/lexer should match.
//!
//! Also of note is the `.delimiter()` and the `.or_until()` methods of `many()`,
//! as well as the `escape()` built in parser.

use parsely::{any, end, int, none_of, Lex, Parse, ParseResult};

// This csv parser will output a sequence of integers
//
// The return type basically means "return some type that Parses a Vec<i32>".
// This avoids having to name the type which would be difficult!
fn number_csv() -> impl Parse<Output = Vec<i32>> {
    ...
    int::<i32>()
        ...
        // the range .. means any number of matches (including 0) are accepted
        .many(..)
...
fn string_csv() -> impl Parse<Output = Vec<String>> {
    ...
    let value = none_of(",")
        ...
        // we aren't outputting a Vec because none_of is a lexer not a parser
        // many will simply keep lexing until it fails
        // and then split the input str at that point
        .many(1..)
        // here we use map to transform the **lexer** into a parser
        // we'll turn each &str slice into a String
        // note, this is the same as doing `.map(|s| s.to_string())` but neater
        .map(str::to_string);

    // now we can parse a single value,
    // we can parse any number of them separated by commas in the usual way
    value.many(..).delimiter(',')
}

// This csv parser will output a sequence of strings but this time
// the strings may contain commas, we enable this by using an **escape sequence**:
//
// The escape sequence is a built in parsely parser
// (you could build your own quite easily though,
// check out the source code of paresly::escape)
//
// It takes an escape character, and an array of escape sequences
// that is the character being escaped and the character it should be transformed into
fn escaped_csv() -> impl Parse<Output = Vec<String>> {
    let value = parsely::escape(
        // the escape character is \
        // note: in rust source code \ itself needs escaping with a \
        '\\',
        [
    ...
    // the escape sequence parser only returns a single character
    // let's parse 1 or more in a row
    .many(1..)
    // many *defaults* to returning a `Vec` for convenience,
    // but it's capable of collecting into any type that implements `std::iter::Extend`
    //
    // .collect() is a method to change the type that many collects into
    // here we are telling the many to collect the chars into a String instead of a Vec
    .collect::<String>()
    // the many parser has no max bound, so it would attempt to convert
    // the whole csv into one String - that's no good!
    //
    // To avoid that we use another method on the `many` combinator: `or_until()`
    // or_until(',') makes the many stop parsing if it "sees" a ','.
    // It "sees" the comma by attempting to lex one after each match,
    // if the lex succeeds then it stops before consuming it
    .or_until(',');
...
// This csv parser will also output a sequence of strings
//
// This time we'll do what a lot of csv parsers do and
// allow values to contain commas if they are quoted
// that is preceeded and followed by a quote char
//
// in order to allow strings to contain literal quotes
// we'll transform "" -> " while inside of a string
// these escaping rules don't match those of parsely's built in escape parser,
// so we'll build our own using a function
fn quote_csv() -> impl Parse<Output = Vec<String>> {
    ...
    let string = '"' // start a string with a double quote
        // we *match* the double quote but we don't want it to be part of our string output,
        // so we *skip* it, *then* parse something, using `.skip_then()`
        .skip_then(
            ...
            repeat_quote_escape
                ...
                .or_until(
                    ...
                    // if not, we should fail since that means it must have been a lone " in
                    // the middle of a value - which is not allowed!
                    .then(
                        // we do that by checking the " was followed by a comma or
                        // the end of the input (which is exactly what the built in
                        // parsely lexer `end()` does)
                        ','.or(end())
                    )
                )
                // since our custom parser outputs chars just like the built in one,
                // we can collect into a String as usual
                .collect::<String>(),
        )
        // we consume the closing double quote, but not include it in our string output
        // so after our parser, we *then* *skip* some lexer,
        // in this case just the double quote
        .then_skip('"');
...
// this is our custom escape function parser that transforms "" -> ""
fn repeat_quote_escape(input: &str) -> ParseResult<char> {
    ...
    match "\"\"".lex(input) {
        ...
        Ok(
            // ignore the double quotes we just matched for now, but keep the remaining input
            (_, remaining),
        ) => Ok(
            // return one double quote character - we've just transformed two quotes into one,
            ('"', remaining), // don't forget to also return the remaining input
        ),
        ...
        Err(_) => {
            ...
            let (matched, remaining) = 
                ...
                any().lex(input)?;  // this ? means we'll return error if input is empty
            // but `any()` doesn't return a `char` for us,
            // it returns `&str` containing one char, so we need to get that char
            Ok((
                // unwrap: any fails if the input is empty
                matched.chars().next().unwrap(),  // this is our output, the next char
                remaining,  // don't forget to also include the remaining input
            ))
        }
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // running number_csv() produces the parser
    // we have to call `.parse(input)` to do the actual parsing
...
pretty neat huh?"
    ...
    assert_eq!(
        ...
        vec![
            ...
            //
            // Can you write a parser that parses quoted csv but also
